...
class MujocoEnv(gymnasium.Env):
    ...
    def __init__(self, model_path, frame_skip, action_type='original', bijective_dims=10, validity_type="none", validity_box_size=0.05,
                 mujoco_env_box_seed=123, valid_box_volume_multiplier=None, **kwargs):
        ...
        if n == 6: 
            self.num_boxes = 6 * 33 # For half cheetah
        ...
        if valid_box_volume_multiplier is not None: 
            volume_per_box *= valid_box_volume_multiplier   # take in ratio as optional input
        elif n == 17:
            volume_per_box *= 20 # For humanoid
        elif n == 3:
            volume_per_box *= 1.65 # For hopper (default 1.65)
        elif n == 6: 
            volume_per_box *= 1   # for half cheetah/walker 2d (default 1)
        elif n == 8: 
            volume_per_box *= 0.5   # for ant (default 1)
        elif n == 2: 
            volume_per_box *= 2   # for swimmer, idp (default 1)
        elif n == 1:
            volume_per_box *= 3 # For pendulums (default 3)
    ...
    def set_state(self, qpos, qvel):
        # Validate the shapes of qpos and qvel
        assert qpos.shape == (self.model.nq,) and qvel.shape == (self.model.nv,)
        
        # Update the state directly in the mjData object
        self.data.qpos[:] = qpos  # Set joint positions
        self.data.qvel[:] = qvel  # Set joint velocities
        
        # Perform a forward pass to update dependent quantities like body positions, velocities, etc.
    ...
    def get_body_com(self, body_name):
         ...
         # Get the body ID by searching for the body name in the model's body_names
         # raise NotImplementedError # need to switch to mujoco 2.0
