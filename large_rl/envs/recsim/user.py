...
class AbstractUserSampler(object):
    ...
    def __init__(self, user_ctor, args=None):
        ...
        self._args = args
        self._seed = args["env_seed"]  # checked!
        self.reset_sampler()
...
class AbstractUserModel(object):
    ...
    def __init__(self, response_model_ctor, user_sampler, slate_size, args=None):
    ...
    def create_observation(self):
        ...
        return self._user_state.create_observation(rng=self._user_sampler._rng)

    def create_empty_response(self, slate_size: int):
        """Returns the user's response after the terminal state.
           Read the logic to use this API in step API in environment.py
        """
        return [self._response_model_ctor() for _ in range(slate_size)]
