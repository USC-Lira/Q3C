""" Greedy AC Implementation: Cascaded Critics operates on the discrete candidates """
...
from large_rl.policy.arch.ddpg import GreedyActor, DDPG_OUNoise as OUNoise, GaussianNoise
...
class GreedyWOLP(Agent):
    def __init__(self, **kwargs):
        super(GreedyWOLP, self).__init__(**kwargs)
        ...
        if self._args["WOLP_if_cem_actor"]:
            ...
            if self._args["WOLP_if_ar"]:
                ...
                self.main_actor = GreedyActor(dim_in=_get_dim_cascade_input(),
                ...
                self.target_actor = deepcopy(self.main_actor) # NOTE: This is unused
                self.sampler_policy = deepcopy(self.main_actor)
                self.opt_actor = torch.optim.Adam(params=self.main_actor.parameters(), lr=self._args["WOLP_actor_lr"])
                self.opt_sampler = torch.optim.Adam(params=self.sampler_policy.parameters(), lr=self._args["WOLP_actor_lr"])
        ...
        for i in range(self._args["CEM_num_iter"]):
            ...
            with torch.no_grad():
                ...
                if self._args["env_name"].lower().startswith("mujoco"):
                    ...
                    Q_vals = q_net(state, torch.tensor(actions, device=self._device)).squeeze(-1)
    ...
    def _select_action(self, input_dict: dict, naive_eval=False, **kwargs):
        ...
        if kwargs.get("if_update", False):
            _, _, action_mean = self.main_actor.sample(input_dict["state_embed"])
            if not self._args['greedy_td3_exploration']:
                q_val = critic(torch.cat([input_dict["state_embed"], action_mean], dim=-1))
                return q_val, q_val.mean(dim=-1)
        else:
            action, _, action_mean  = self.main_actor.sample(input_dict["state_embed"])
            if not self._args['greedy_td3_exploration']:
            ...
            if _if_boltz:
                ...
                if self._args["TwinQ"]:
                    if self._args["TD3_target_policy_smoothing"]:
                        ...
                        Q_val = torch.min(
                            critic(torch.cat([_s[:, 0], target_a.squeeze(1)], dim=-1)),
            ...
            if train_actor:
                if self._args["WOLP_if_ar"]:
                    ...
                    if self._args["env_name"].lower() in ["mine", "recsim-data"] and not self._args['mw_obs_flatten']:
                        _s = self.main_extra_critic_obs_enc(input_dict["state_embed"])
                    Q = self.main_extra_critic(torch.cat([_s, mu], dim=-1))
                elif self._args["method_name"].lower() != "greedy_ac":
                ...
                if self._args["method_name"].lower() == "greedy_ac":
                    # GreedyAC
                    if self._args["env_name"].lower() in ["mine", "recsim-data"] and not self._args['mw_obs_flatten']:
                        _s = self.main_actor_obs_enc(input_dict["state_embed"])
                        assert False, "Needs dimensional verification!"
                        ...
                        _s = input_dict["state_embed"]
                    # Sample actions from the sampler to determine which to update with
                    action_batch, _, _ = self.sampler_policy.sample(_s, self._args['greedy_N'])
                    action_batch = action_batch.permute(1, 0, 2)
                    action_batch = action_batch.reshape(_s.shape[0] * self._args['greedy_N'],
                                                        -1)
                    stacked_s_batch = _s.repeat_interleave(self._args['greedy_N'],
                                                                    dim=0)
                    # Get the values of the sampled actions and find the best
                    # ϱ * num_samples actions
                    with torch.no_grad():
                        q_values = self.main_ref_critic(torch.cat([stacked_s_batch, action_batch], dim=-1))

                    q_values = q_values.reshape(_s.shape[0], self._args['greedy_N'],
                                                1)
                    sorted_q = torch.argsort(q_values, dim=1, descending=True)
                    best_ind = sorted_q[:, :self._args['greedy_Nrho']]
                    best_ind = best_ind.repeat_interleave(action_batch.shape[-1], -1)
                    ...
                    stacked_s_batch = _s.repeat_interleave(self._args['greedy_Nrho'], dim=0)
                    best_actions = torch.reshape(best_actions, (-1, action_batch.shape[-1]))

                    # Actor loss
                    policy_loss = self.main_actor.log_prob(stacked_s_batch, best_actions)
                    ...
                    stacked_s_batch = _s.repeat_interleave(self._args['greedy_N'], dim=0)
                    stacked_s_batch = stacked_s_batch.reshape(-1, _s.shape[-1])
                    action_batch = action_batch.reshape(-1, action_batch.shape[-1])

                    sampler_entropy = self.sampler_policy.log_prob(stacked_s_batch, action_batch)
                    with torch.no_grad():
                        sampler_entropy *= sampler_entropy

                    sampler_entropy = sampler_entropy.reshape(_s.shape[0],
                                                            self._args['greedy_N'], 1)
                    if self._args["greedy_entropy_from_single_sample"]:
                    ...
                    stacked_s_batch = _s.repeat_interleave(self._args['greedy_Nrho'], dim=0)
                    sampler_loss = self.sampler_policy.log_prob(stacked_s_batch, best_actions)
                    sampler_loss = sampler_loss.reshape(_s.shape[0], self._args['greedy_Nrho'], 1)
                    sampler_loss = sampler_loss.mean(axis=1)
                    sampler_loss = sampler_loss + (sampler_entropy * self._args['greedy_alpha'])
                    sampler_loss = -sampler_loss.mean()

                    # Update the sampler
                    self.opt_sampler.zero_grad()
                    sampler_loss.backward()
                    if self._args["if_grad_clip"]: clip_grad(self.sampler_policy.parameters(), 1.0)  # gradient clipping
    ...
    def _save(self, save_dir):
        ...
        torch.save(self.sampler_policy.state_dict(), os.path.join(save_dir, f"sampler_policy.pkl"))
        torch.save(self.opt_actor.state_dict(), os.path.join(save_dir, f"opt_actor.pkl"))
        torch.save(self.opt_sampler.state_dict(), os.path.join(save_dir, f"opt_sampler.pkl"))
    ...
    def _load(self, load_dir):
        ...
        self.sampler_policy.load_state_dict(torch.load(os.path.join(load_dir, f"sampler_policy.pkl")))
        self.opt_actor.load_state_dict(torch.load(os.path.join(load_dir, f"opt_actor.pkl")))
        self.opt_sampler.load_state_dict(torch.load(os.path.join(load_dir, f"opt_sampler.pkl")))
